...
// 本文件主要是获取webpack-config.js文件，和执行命令行命令
...
module.exports = function(...args) {
	...
	// 这种参数默认配置里没写有，但是可以通过命令行方式输入，-d="值"  或 --d="值"
	if (argv.d) {
		argv.debug = true;
	...
	let configFileLoaded = false; // 配置文件加载进度标志
	let configFiles = [];
	...
	/*
		extensions = [
			...
			'.node',     '.toml',      '.wisp',
			'.yaml',     '.cirru',     '.json5',
			'.coffee',   '.esm.js',    '.iced.md',
			'.liticed',  '.babel.js',  '.babel.ts',
			'.buble.js', '.coffee.md', '.litcoffee'
		]
	*/
	let i;
	// argv.config 默认为undefined 除非手动在命令行配置
	if (argv.config) {
		...
		// webpack.config.js | webpackfile.js默认的webpack配置文件
		const defaultConfigFileNames = ["webpack.config", "webpackfile"].join("|");
		// webpackConfigFileRegExp = (webpack.config|webpackfile)(.js|.co|.eg|.ls|.ts|.csv|.ini|.jsx|.mjs|.tsx|.xml|.yml|.cjsx|.iced|.json|.node|.toml|.wisp|.yaml|.cirru|.json5|.coffee|.esm.js|.iced.md|.liticed|.babel.js|.babel.ts|.buble.js|.coffee.md|.litcoffee
		const webpackConfigFileRegExp = `(${defaultConfigFileNames})(${extensions.join("|")})`;
		// pathToWebpackConfig = H:\不回头\webpack\webpack-demo\webpack.config.js
		const pathToWebpackConfig = findup(webpackConfigFileRegExp);

		if (pathToWebpackConfig) {
			// resolvedPath = H:\不回头\webpack\webpack-demo\webpack.config.js
			const resolvedPath = path.resolve(pathToWebpackConfig);
			// actualConfigFileName = webpack.config.js
	...
	if (configFiles.length > 0) {
		...
		/*
			configFiles = [
				{
					path: 'H:\\不回头\\webpack\\webpack-demo\\webpack.config.js',
					ext: '.js'
				}
			]
		*/
		configFiles.forEach(function(file) {
			// interpret.extensions[file.ext] = null  <==> ".js": null
			/* 
				requireConfig(file.path) = {
					mode: 'production',
		...
		configFileLoaded = true; // 配置文件加载完成
	}
	...
	function processConfiguredOptions(options) {
		if (options) {
			validateOptions(options); // 验证options
		} else {
			options = {};
		}

...
// 上面这种链式调用太麻烦，可以用.options(key, [opt])，效果一样的
    ...
    yargs
        ...
        .options({
			config: {
				...
				group: CONFIG_GROUP, // .group(key(s), groupName) 把一个key或者一组keys放到一个 groupName下，当调用使用文档的时候会分组展示
			...
			mode: {
				...
				choices: getSchemaInfo("mode", "enum"), // 可选值
